              ...
              {items.map((item) => (
                ...
                >
                  ...
                  <td className="px-1 py-4 w-full justify-center flex gap-4">
                    {item.active?(<button onClick={()=>{handleActive(false,item)}}>Deactivate</button>):(<button onClick={()=>{handleActive(true,item)}} >Activate</button>)}
                    {/* <Switch
                      onChange={(e) => handleToggle(e,item.id)}
                      checked={item.isActive}
                      onColor="#86d3ff" // Color when the switch is ON
                      onHandleColor="#2693e6" // Color of the switch's handle when ON
                      handleDiameter={24}
                      uncheckedIcon={false}
                      checkedIcon={false}
                      height={20}
